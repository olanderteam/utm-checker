...
function buildPlatformBlock(label, { count, byCampaign }) {
...
export function buildLeadsStatus({ total, meta, google, others }) {
  if (total === 0) {
...
export function buildSlackBlocks(timestamp, leadsAnalysis) {
  const blocks = [
    ...
    {
      type: 'context',
      elements: [{ type: 'mrkdwn', text: `Disparo às 8h e 18h (Brasília) | Dados via Supabase` }],
    }
  ];
