...
export function buildSlackBlocks(timestamp, leadsAnalysis, attentionPoints = [], options = {}) {
  const { periodLabel = 'hoje', headerTitle = '🎯 SQLs do Dia — [OKR][2025Q4]', correctionMode = false } = options;

  const headerText = correctionMode
    ? `📡 UTM Bot em Modo de Correção — ${timestamp}`
    : `📡 UTM Bot — ${timestamp}`;
  ...
  if (correctionMode) {
    blocks.push({
      type: 'context',
      elements: [{ type: 'mrkdwn', text: '🔧 Bot em observação após correção de bug — monitorando por alguns dias para confirmar estabilidade.' }],
    });
  }
